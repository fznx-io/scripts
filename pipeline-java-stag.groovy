    ...
    stages {
        stage("Fetching The Code & Config") {
            steps {
                ...
                sh "cp -r $appconfig_stag/$service_name-$env/* ."
                sh "cp -r $appconfig_stag/$service_name-$env/resources/* src/main/resources/"
        ...
        stage("Building The Image") {
            steps {
                sh "docker build -t $imagerepo_stag/$service_name:$env.$build_number ."
                sh "docker tag $imagerepo_stag/$service_name:$env.$build_number $imagerepo_stag/$service_name:$env.$latest"
            }
        }

        stage("Pushing to The Registry") {
            steps {
                sh "docker push $imagerepo_stag/$service_name:$env.$build_number"
                sh "docker push $imagerepo_stag/$service_name:$env.$latest"
            }
        }

        stage("Deploying to The Cluster") {
            steps {
                sh "kubectl set image deployment/$service_name $service_name=$imagerepo_stag/$service_name:$env.$build_number --kubeconfig=$kubeconfig_stag"
            }
        }
    }
    post{
