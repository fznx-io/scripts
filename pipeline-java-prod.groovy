    ...
    stages {
        stage("Fetching The Code & Config") {
            steps {
                ...
                sh "cp -r $appconfig_prod/$service_name-$env/resources/* src/main/resources/"
        ...
        stage("Deploying to The Cluster") {
            steps {
                sh "kubectl set image deployment/$service_name $service_name=$imagerepo_prod/$service_name:$env.$build_number --kubeconfig=$kubeconfig_prod"
            }
        }
    }
    post{
